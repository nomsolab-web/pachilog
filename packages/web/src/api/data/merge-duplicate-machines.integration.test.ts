...
describe("duplicate machine merge integration", () => {
  beforeAll(async () => {
    ...
    await client.batch([
      ...
      "CREATE TABLE video_machine_links (id integer PRIMARY KEY AUTOINCREMENT, video_id text NOT NULL, machine_id integer NOT NULL, match_confidence integer NOT NULL, match_method text NOT NULL, created_at integer NOT NULL, updated_at integer NOT NULL, UNIQUE(video_id, machine_id))",
      "CREATE TABLE machine_mentions (id integer PRIMARY KEY AUTOINCREMENT, machine_id integer NOT NULL, channel_id integer NOT NULL, video_id text NOT NULL, video_title text NOT NULL, view_count integer NOT NULL, like_count integer NOT NULL, comment_count integer NOT NULL, published_at text, updated_at integer NOT NULL, UNIQUE(machine_id, video_id))",
      "CREATE TABLE machine_votes (id integer PRIMARY KEY AUTOINCREMENT, machine_id integer NOT NULL, vote_type text NOT NULL, voter_fingerprint text NOT NULL, created_at integer NOT NULL, UNIQUE(machine_id, voter_fingerprint))",
      "CREATE TABLE machine_video_judgments (id integer PRIMARY KEY AUTOINCREMENT, judgment_key text NOT NULL UNIQUE, machine_id integer, channel_id integer, video_id text NOT NULL, video_title text NOT NULL, channel_name text, published_at text, source text NOT NULL, status text NOT NULL, confidence integer NOT NULL, reason text, matched_terms text, raw_response text, created_at integer NOT NULL, updated_at integer NOT NULL)",
    ]);
  });
