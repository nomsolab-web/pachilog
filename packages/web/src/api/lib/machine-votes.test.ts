...
import { isMachineVoteType, isPlainRecord, machineVoteStatus, validateVoterFingerprint } from "./machine-votes";
...
describe("machine vote helpers", () => {
  ...
  test("recognizes only non-null plain object bodies", () => {
    expect(isPlainRecord({ voteType: "want_to_play" })).toBe(true);
