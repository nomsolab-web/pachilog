...
- **移行検討中:** RunableのStarterプラン($25/月)のコストを踏まえ、Google Cloud Run(無料枠)への自前ホスティング移行を検討中。[DEPLOY_GCP.md](./DEPLOY_GCP.md)に手順書あり、**ただし未実施・Dockerfileはビルド未検証**
...
3. **Cloud Run移行の実施(検討中、未実施)**
   - [DEPLOY_GCP.md](./DEPLOY_GCP.md)に手順書あり
   - `Dockerfile`はサンドボックスにDockerが無く**ビルド未検証**。まずローカルで`docker build`が通るか確認すること
   - 移行するかどうかは運営者がRunableの契約状況(残り日数など)を見て判断中
